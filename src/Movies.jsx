...
const Movies = () => {
  ...
  const cast_members = movieDetails?.credits?.cast;

  // Return names of actors
  let actingCast = [];
  if(cast_members) {
    actingCast = cast_members
    .filter((member) => member.known_for_department.toLowerCase() === "acting")
    .slice(0, 3);
  }
  const actorNames = actingCast.map((actor) => actor.name).join(", ");
          ...
          <div className="flex flex-col gap-y-5 w-2/3">
            <div className="flex gap-x-4 items-center">
              ...
              <p className="font-bold text-xl">
                <span className="font-normal" data-testid="movie-title">
                  {title}
                </span>{" "}
                &middot;{" "}
                <span className="font-normal" data-testid="movie-release-date">
                  {release_date}
                </span>{" "}
                &middot;{" "}
                <span className="font-normal" data-testid="movie-runtime">
                  {runtime}m
                </span>
              </p>
              {/* movie genres container */}
              <div className="flex gap-x-3">
                <span className="border border-solid border-pink-100 rounded-full text-red-700 py-1 px-5 h-8">
                  Drama
                </span>
                <span className="border border-solid border-pink-100 rounded-full text-red-700 py-1 px-5 h-8">
                  Crime
            ...
            <div className="flex flex-col gap-y-7 text-xl">
              ...
              <p>
                ...
                Director:{" "}
                <span className="text-rose-700">Francis Ford Coppola</span>
              </p>
              <p>
              ...
              <p>
                {/* stars */}
                Stars: <span className="text-rose-700">{actorNames}</span>
              </p>
            </div>
